...
```{r Logistic Regression Model fit, cache=TRUE,cache.lazy=FALSE,warning=FALSE,message=FALSE,eval=TRUE}
...
```{r ridge regression model,cache=TRUE,cache.lazy=FALSE,warning=FALSE,message=FALSE,eval=TRUE}
...
```{r read in Ridge/Lasso Regression Model,echo=FALSE,eval=FALSE}
...
```{r}
lr_RL_best<-select_best(lr_pen_fit,metric = "roc_auc")
...
```{r load RF model, eval=FALSE,echo=FALSE}
