...
train%>%
  ...
  ggpairs(mapping=aes(color=Response),title="Numeric Predictors as a Function of Response")+
...
4. Some categorical predictors have either few responses in one category (Driver's License) or have many categories that need to be consolidated (REgion code and Policy Sales Channel)
...
+Pros
  *Same as logistic regression but can accomodate highly correlated predictors with penalty/mixture parameters
...
I apply a logorithmic transformation of the annual premium predictor as it is heavily skewed, and 
...
regression models require the  factor predictors to be converted to dummy variables. Finally I use step nsv 
to remove any predictors that are highly unbalanced such as the driver's license predictor. Finally, I
...
```{r Logistic Regression Model fit,warning=FALSE,message=FALSE,eval=TRUE}
...
```{r ridge regression model,warning=FALSE,message=FALSE,eval=TRUE}
...
```{r random forest model,warning=FALSE,message=FALSE,eval=TRUE}
set.seed(1234)
...
## Upsample model fit
```{r upsample model fit,eval=TRUE}
...
```{r downsample model fit,eval=TRUE}
set.seed(1234)
c1 <- makePSOCKcluster(5)
registerDoParallel(c1)
...
```{r downsample final model parameters}
