...
upsamp_lr_fit %>%
  ...
  ggplot(aes(over_ratio, mean, color=over_ratio)) +
  geom_point(size=2) +
  facet_wrap(~ .metric, scales = "free", nrow = 2) +
...
downsamp_lr_fit %>%
  ...
  mutate(under_ratio = factor(under_ratio),
         metric=factor(.metric)) %>%
  ggplot(aes(under_ratio, mean, color=under_ratio)) +
  geom_point(size=2) +
  facet_wrap(~ .metric, scales = "free", nrow = 2) +
  scale_color_viridis_d()
```
...
upsamp_best<-select_best(upsamp_lr_fit,metric='spec')
downsamp_best<-select_best(downsamp_lr_fit,metric='spec')
smote_best<-select_best(smote_lr_fit,metric='spec')
...
up_curve<-upsamp_pred%>%
  roc_curve(Response,.pred_No)%>%
  mutate(model="Best Upsample")
down_curve<-downsamp_pred%>%
  roc_curve(Response,.pred_No)%>%
  mutate(model="Best Downsample")
smote_curve<-smote_pred%>%
  roc_curve(Response,.pred_No)%>%
  mutate(model="Best SMOTE")
