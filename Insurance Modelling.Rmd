...
```{r load RF model, eval=TRUE,echo=FALSE}
...
capture the "Yes" responses. This is likely due to the unbalanced nature of the outcome
...
upsamp_grid<-grid_regular(over_ratio()%>%range_set(c(.1,2.0)),
                          levels=20)
downsamp_grid<-grid_regular(under_ratio()%>%range_set(c(.1,2.0)),
                            levels=20)
smote_grid<-grid_regular(over_ratio()%>%range_set(c(.1,2.0)),
                         neighbors()%>%range_set(c(2,10)),
                         levels=20)
...
upsamp_lr_fit %>%
  collect_metrics() %>%
  mutate(over_ratio = factor(over_ratio),
         metric=factor(.metric)) %>%
  ggplot(aes(over_ratio, mean, fill=over_ratio)) +
  geom_col() +
  facet_wrap(~ .metric, scales = "free", nrow = 2) +
  scale_fill_viridis_d(begin=.9,end=0)
```
...
downsamp_lr_fit %>%
  ...
  mutate(under_ratio = factor(under_ratio),
         metric=factor(.metric)) %>%
  ggplot(aes(under_ratio, mean, fill=under_ratio)) +
  geom_col() +
  facet_wrap(~ .metric, scales = "free", nrow = 2) +
  scale_fill_viridis_d(begin=.9,end=0)
```
...
smote_lr_fit %>%
  ...
  ggplot(aes(over_ratio, mean, fill=neighbors)) +
  geom_line() +
  geom_point()+
  facet_wrap(~ .metric, scales = "free", nrow = 2) +
  scale_fill_viridis_d(begin=.9,end=0)
